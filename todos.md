...
- `86` TODO matches currently exist under `src`.
- Most TODOs are TypeScript port gaps, not upstream ACadSharp gaps.
- The highest concentration is now in entity transform helpers, the remaining dimension block/override work, and a small set of DWG object-reader gaps.

## Highest Concentration

- `7` in [src/Entities/Entity.ts](src/Entities/Entity.ts)
- `6` in [src/Entities/Hatch.ts](src/Entities/Hatch.ts)
- `5` in [src/Entities/Dimension.ts](src/Entities/Dimension.ts)
- `3` in [src/Entities/DimensionAligned.ts](src/Entities/DimensionAligned.ts)
...
- `2` in [src/Entities/Spline.ts](src/Entities/Spline.ts)
- `2` in [src/Entities/MText.ts](src/Entities/MText.ts)

## TS-Only Backlog

- Geometry and transform helpers are still missing in the TS port. This now concentrates mostly in [src/Entities/Entity.ts](src/Entities/Entity.ts), [src/Entities/Hatch.ts](src/Entities/Hatch.ts), [src/Entities/Insert.ts](src/Entities/Insert.ts), [src/Entities/Polyline.ts](src/Entities/Polyline.ts), and related entity files.
...
- Hatch explode/transform support is still largely unported in [src/Entities/Hatch.ts](src/Entities/Hatch.ts), but boundary-edge entity conversion, spline conversion, and path bounds are implemented now.
...
- Continue with the remaining dimension override-map and block-generation backlog in [src/Entities/Dimension.ts](src/Entities/Dimension.ts) and the transform-heavy geometry backlog in [src/Entities/Entity.ts](src/Entities/Entity.ts), [src/Entities/Hatch.ts](src/Entities/Hatch.ts), [src/Entities/Insert.ts](src/Entities/Insert.ts), and the dimension subtype files.
- Revisit [src/Entities/Spline.ts](src/Entities/Spline.ts) for the remaining transform and fit-point algorithm TODOs once a safe vector-math port is in scope.
